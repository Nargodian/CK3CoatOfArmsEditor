...
uniform sampler2D emblemMask;        // The emblem mask atlas (RGB channels for coloring)
uniform sampler2D patternMask;       // Pattern atlas texture for mask channels
uniform sampler2D texturedMask;      // Material/dirt texture (coa_mask_texture.png)
uniform sampler2D noiseMask;         // Noise texture (noise.png)
...
void main()
{
	vec4 textureMask = texture(emblemMask, vTexCoord);
	...
	vec4 patternSample = texture(patternMask, patternCoord);
	// flags
	// 0 mask off
	// 1 maskR on
	// 2 maskG on
