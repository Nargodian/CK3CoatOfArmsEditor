...
if __name__ == "__main__":
	...
	# Parse sample 1
	try:
		data1 = parse_coa_file("coa_sample_1.txt")
		print("\n✓ Successfully parsed coa_sample_1.txt")
		print(f"  Pattern: {list(data1.values())[0].get('pattern')}")
		print(f"  Colors: {list(data1.values())[0].get('color1')}, {list(data1.values())[0].get('color2')}, {list(data1.values())[0].get('color3')}")
		print(f"  Emblems: {len(list(data1.values())[0].get('colored_emblem', []))}")
		
		# Test round-trip
		serialized = serialize_coa_to_string(data1)
		data1_reparse = parse_coa_string(serialized)
		print("  ✓ Round-trip successful")
		
	except Exception as e:
		print(f"\n✗ Error parsing coa_sample_1.txt: {e}")
		import traceback
		traceback.print_exc()
	
	# Parse sample 2
	try:
		data2 = parse_coa_file("coa_sample_2.txt")
		print("\n✓ Successfully parsed coa_sample_2.txt")
		print(f"  Pattern: {list(data2.values())[0].get('pattern')}")
		print(f"  Colors: {list(data2.values())[0].get('color1')}, {list(data2.values())[0].get('color2')}, {list(data2.values())[0].get('color3')}")
		print(f"  Emblems: {len(list(data2.values())[0].get('colored_emblem', []))}")
		
		# Test round-trip
		serialized = serialize_coa_to_string(data2)
		data2_reparse = parse_coa_string(serialized)
		print("  ✓ Round-trip successful")
		
	except Exception as e:
		print(f"\n✗ Error parsing coa_sample_2.txt: {e}")
		import traceback
		traceback.print_exc()
	
	print("\n" + "=" * 60)
	print("Parser/Serializer ready for use!")
